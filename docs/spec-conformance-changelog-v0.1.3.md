# CityG Spec Conformance Changelog (v0.1.3)

> [!IMPORTANT]
> Historical profile-transition archive for the retired `v0.1.3` wire/profile revision.
...
Note on versioning:
- The implementation now advertises wire/API `profile_version = v0.1.3`.
- `v0.1.3` is a real wire-profile revision from `v0.1.2`, not a repository-only errata layer.
...
The entries below include the carried-forward `v0.1.2` baseline and the `v0.1.3` profile revision point.
...
| 2026-03-20 | `TBD` | Added `join_finalize` reason-2 flow and bumped the advertised wire profile to `v0.1.3` | S10.4-S10.4C, S11.11.1, S11.11.3, S11.12.1, S11.14.2, S12.2, S12.3, S14.7-S14.8 | Lets a pending joiner self-finalize into a message-ready state without another client online, while keeping PCS rate-limit and `K_fs` reseed semantics scoped to reason 1 only. |
...
- S14 manifest path: `kat/kat-s14-conformance-manifest-v0.1.3.json`
- Freeze-blockers manifest path: `kat/kat-freeze-blockers-manifest-v0.1.3.json`
