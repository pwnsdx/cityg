...
- [~] Add KATs for slot reuse after leave/revoke
  `kat/kat-slot-lease-conformance-v0.2.json` now maps the shipped deterministic tests for reclaim clearing, stale `join_finalize_auth` rejection, helper generation binding, public join/merge ticket tamper rejection, API-level stale `join_finalize_auth` rejection after slot reuse, public reclaim acceptance lookup transitions, live telemetry checks both during pending reclaim and after reclaim acceptance, and a serialized end-to-end reclaim fixture (`kat/kat-slot-lease-reclaim-e2e-v0.2.json`) replayed against a live HTTP flow. `scripts/run_slot_lease_conformance.sh` provides a repeatable runner for the current slot-lease suite and now runs cleanly end-to-end locally; `slot_lease_manifest_is_well_formed_and_complete` validates that the manifest still points at real tests, and `slot_lease_conformance_runner_covers_manifest_tests` validates that the runner still covers every manifest entry. Broader fixture-style KATs are still pending.
