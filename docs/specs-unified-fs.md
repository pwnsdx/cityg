...
Require `header[143] == T_base`; else **Freeze `945.0 fs_base_mismatch`**.
...
* `132 rollup_provenance_commit` — **REQUIRED**:
  `H_L("msphf/rollup/prov", CBOR_det([[weid,vck,xk_hash],…]))`.
...
* **SRX/Smallwood‑v1 (if required):** verify `161` binds the shadow roots to on‑wire rpo‑256 roots (`112/113`) per Annex F; `125` must include `160/161`.
...
**Purpose.** Provide a **zero‑knowledge** proof that the on‑wire SRX roots `112/113` reflect a valid set‑delta application, without revealing which leaves changed.
