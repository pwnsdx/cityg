...
`city-g/v0.4` itself: a catch-up that a stolen device key signs, under the
rule the profile had, and the fix that the note
[`preuves-et-mesures-2026-09-26.md`](../preuves-et-mesures-2026-09-26.md)
(in French) proposed, which the profile now has (its own model holds the
same pair). None of the other mechanisms is part of the profile. The model of the protocol itself is
