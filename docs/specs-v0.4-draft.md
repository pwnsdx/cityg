# City-G protocol specification — profile v0.4 (draft)
...
interpreted as in RFC 2119 and RFC 8174 when they appear in capitals. In
this draft they state what the prototype does and what a normative profile
would require; they bind nobody yet.

Profile v0.4 ("Cité") is built for groups of millions of members, waves of
hundreds of thousands of joins and departures, and groups where no member
may be online for long periods. It keeps the suite, the encodings and the
key schedule of v0.3, and changes how the group is re-keyed:
...
20. [Differences from profile v0.3](#20-differences)

<a id="1-architecture"></a>
## 1. Architecture

* **Device and occupancy.** As in v0.3: a member device holds an ML-DSA-65
  device key per group, and occupies one leaf from the epoch it entered,
  `since`. The pair `[leaf, since]`, its *occupancy*, names the member for
  good.
...
The adversaries A1 to A6 of v0.3 (section 2.1 of the v0.3 specification)
are unchanged. The properties of v0.3 section 2.2 carry over for members
that follow the group, with the changes below.

* **Post-removal secrecy** holds from the epoch of the window that applies
  the removal. A removal is recorded when the DS accepts its proposal and
  applied by the next window (section 14.2). Until then the DS refuses the
  removed member's messages and commits and stops delivering to it
  (section 14.6); this enforcement is not cryptographic. A member MUST NOT
  send in an epoch in which a removal recorded more than `WINDOW_REMOVAL`
  ago still waits (section 12.8).
* **A removed committer** keeps no access through the nodes it drew: a
  window that removes a member re-keys every node it taints (section 10.2;
  model `taint.pv`).
* **Forward secrecy** of past epochs comes from the init chain (section 9):
  a leaked leaf key does not open past epochs (model
  `forward_secrecy.pv`).
* **Post-compromise security** of a device comes from its next key update
  (section 12.6), which re-keys its path and every node it taints (model
  `post_compromise.pv`). Members SHOULD update at least every
  `UPDATE_INTERVAL`.
* **Membership agreement** for members that follow the group rests on the
  confirmation tag: `GroupContext` binds the tree, the registry and the
  transcript (section 9). For a window sealed by an entrant, the tag alone
...
Assumptions and limits, in addition to those of v0.3 section 2.3:

* A committer learns the secrets of the nodes it re-keys, including off its
  own path. An honest committer erases them once its commit is sent
  (section 12.4); taints bound the damage of one that does not.
* An entrant learns every secret of the window it seals. It is a member of
  the new epoch, admitted by an admin or already a member.
* In a window sealed by an entrant, the external init secret is known to
  ...
  window re-keys away from them (model `entrant_removal.pv`).
...
The suite is v0.3's (v0.3 section 3): X-Wing, ML-DSA-65 with context
strings, BLAKE3, ChaCha20-Poly1305. Encodings use `CBOR_det` (v0.3
section 4.1). The derivation functions are v0.3's with the framing tags of
this profile, so that no hash, key or MAC of one profile can be taken for
one of the other:
...
**Signed arrays.** Every signed object is
`CBOR_det([label, field_2, ..., field_k, signature])`, where `signature`
is ML-DSA-65 over `CBOR_det([label, field_2, ..., field_k])` under the
object's context (section 17.3). The label names the object and its
version; every label of this profile ends in `/v4`. A verifier checks the
label and the number of fields before the signature. Seals are signed
differently (section 10.4).
...
changed leaf. The prototype never shrinks the tree (section 19).

<a id="6-requests"></a>
## 6. Signed requests

Every request is a signed array (section 3), except the eviction, which
...
* **Invite.** Signed by the inviter, an admin (`inviter` its occupancy,
  ...
  seed shared out of band, as in v0.3. It is valid for a window if the
...
The schedule of v0.3 section 8, one epoch per window, with the root secret
...
* The message plane is v0.3's (v0.3 section 11), keyed by `msg_secret_n`;
  the prototype does not implement it (section 19).
...
* **Member windows (kind 1).** The sealer and every committer MUST be
  ...
  never removes or updates itself, rule C-03 of v0.3). Their device keys
  come from the tree.
...
This is O(log N) whatever the size of the group. There are no light
members: every member is light in the sense of v0.3 section 14.
...
prototype checks the whole state; a deployment would send the districts
concerned with proofs to the tree hash (section 19).
...
This draft fixes the content of these objects, not yet their encoding
(section 19). Sizes below are those of the prototype's measurements.
...
Welcomes are accepted for a request the window welcomes. Packets, links and
entries are served as in section 13.
...
No single device can check every entry of a large wave: two signatures per
join take about 210 s of CPU for 500,000 joins. Checking is split (E-12):
...
| Name | Value |
| --- | --- |
| `L` (`district_bits`) | 12 by default, fixed at genesis (the prototype's tests use 2) |
| `WINDOW_MAX` | 60 s |
| `WINDOW_REMOVAL` | 5 s |
...
**Measured costs.** The prototype's scale test
(`crates/cityg-cite/tests/scale.rs`, one core) builds a full group and
runs a wave of half removals and half joins paired with them. The number
of wraps and new keys equals the count of the research cost model
(`rekey_sim.py`) for the same leaves:
...
| `city-g/update/v4` | UpdateRequest |
...
| `city-g/remove/v3` | RemoveProposal (label `city-g/remove/v4`) |

The last four contexts are shared with profile v0.3. The signed bytes of the
two profiles never coincide, since their first fields, the labels, differ;
a normative profile MAY still give them contexts of their own.
...
* **Committers and entrants.** They learn the secrets they draw. Erasure is
  ...
  `taint_without_rule.pv` shows the attack without the rule, and the
  prototype's test `removing_a_committer_rekeys_the_nodes_it_drew` shows it
  on real commits.
...
* **Audits are probabilistic.** An invalid entry that escapes the DS, its
  ...
  per entry, if members audit honestly.
* **Forks.** As in v0.3, the DS decides what each member sees and can split
  the group into branches. Joiners anchor on admin checkpoints and check
  the chain of seals (`anchored_join.pv`), which the v0.3 joiner did not
...
Before this draft could become a normative profile:
...
* the HTTP API, persistence, the worker and the clients; the message plane
  (v0.3 section 11) under the v0.4 framing tags;
...
<a id="20-differences"></a>
## 20. Differences from profile v0.3

| | v0.3 | v0.4 draft |
| --- | --- | --- |
| Tree | array layout, at most 8192 leaves, unmerged leaves and resolutions | sparse, up to `2^24` leaves, districts under a city, blank iff empty, taints |
| Commit | one signed commit per epoch, by one author, at most 64 joins and 256 removals | one window per epoch: district commits in parallel, a seal, welcomes; no cap per window |
| Who commits | a member, or a joiner through an external commit | any member, for any district; an entrant when nobody is online |
| Removals | applied by the next commit | recorded, enforced at delivery, applied by the next window |
| Registry | capacity, admins, up to 4096 retired admissions | admins and two sparse Merkle maps; admissions used once |
| Members | full members (whole tree) or light members | every member keeps its path and headers; one packet per window |
