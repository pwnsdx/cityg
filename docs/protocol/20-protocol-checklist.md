...
| Systematic hostile mutation coverage spans barrier headers, manifests, and sender binding | The runtime must reject mutated barrier payloads, mismatched authority headers, missing manifests, and sender-binding spoofing without corrupting later recovery. | Covered | `hostile_barrier_update_mutations_fail_closed_without_poisoning_restart_recovery` in [`../../crates/cityg-server/src/lib.rs`](../../crates/cityg-server/src/lib.rs), `native::tests::epoch_sync_rejects_barrier_bundle_history_commitment_mismatch`, `native::tests::epoch_sync_rejects_barrier_bundle_fs_policy_version_mismatch`, `native::tests::perform_fetch_rejects_sender_leaf_spoofing_with_mismatched_public_key`, `barrier_fetch_public_tree_rejects_missing_deployment_profile_manifest` in [`../../crates/cityg-api-client/src/lib.rs`](../../crates/cityg-api-client/src/lib.rs) |
