...
| Honest publish wins over a malformed refresh race | A malformed refresh-shaped payload racing with a later honest join must fail closed without overriding the healthy live room, and the room must still survive restart afterward. | Covered | `malformed_refresh_concurrent_with_honest_join_preserves_live_state`, `malformed_refresh_concurrent_with_honest_join_does_not_poison_restart_recovery` in [`../../crates/cityg-server/src/lib.rs`](../../crates/cityg-server/src/lib.rs) |
...
| Honest recovery survives hostile `join_finalize` interference | A malformed `join_finalize` must not strand the later honest recovery path: a published `join_finalize` can still recover through `epoch_sync`, including across restart. | Covered | `native::tests::join_finalize_fault_injection_after_publish_recovers_via_epoch_sync`, `native::tests::restart_during_pending_join_finalize_activation_recovers_via_epoch_sync`, `malformed_join_finalize_rejection_does_not_poison_restart_recovery` in [`../../crates/cityg-server/src/lib.rs`](../../crates/cityg-server/src/lib.rs) |
...
| Base-profile helper guards reject unexpected completeness/authority extensions | Helper responses with unsupported completeness attestations or local-authority extensions must fail closed under the base profile. | Covered | `barrier_resolve_revoked_occupancies_rejects_unexpected_completeness_attestation`, `barrier_resolve_join_occupancies_since_rejects_unexpected_completeness_attestation`, `barrier_fetch_public_tree_rejects_local_history_authority_in_base_profile` in [`../../crates/cityg-api-client/src/lib.rs`](../../crates/cityg-api-client/src/lib.rs) |
| Large public-room churn remains live by composition | Across moderate fanout, the current suite covers expel, survivor refresh, offline catch-up, new joins under churn, same-identity rejoin, replay-state after restart, and live watch recovery as one composite public-room guarantee. | Covered | `native::tests::admin_expel_then_survivor_refresh_preserves_room_and_new_joiner_messaging`, `native::tests::offline_member_restart_then_epoch_sync_after_pcs_refresh_decrypts_new_messages`, `native::tests::rejoin_with_same_persisted_identity_succeeds_after_room_becomes_empty`, `native::tests::dual_restarted_watchers_fetch_offline_backlog_and_resume_live_notifications`, Flow E / Flow G in [`../evidence/e2e-local-validation-2026-03-27.md`](../evidence/e2e-local-validation-2026-03-27.md) |
| Systematic hostile mutation coverage spans barrier headers, manifests, helper pagination, and sender binding | The runtime must reject mutated barrier payloads, mismatched authority headers, missing manifests, paginated helper inconsistencies, and sender-binding spoofing without corrupting later recovery. | Covered | `hostile_barrier_update_mutations_fail_closed_without_poisoning_restart_recovery`, `hostile_barrier_update_byte_flip_sweep_fail_closed_without_poisoning_restart_recovery`, `prop_authority_bound_refresh_bundle_mutations_fail_closed_without_poisoning_live_state` in [`../../crates/cityg-server/src/lib.rs`](../../crates/cityg-server/src/lib.rs), `native::tests::epoch_sync_rejects_barrier_bundle_history_commitment_mismatch`, `native::tests::epoch_sync_rejects_barrier_bundle_fs_policy_version_mismatch`, `native::tests::perform_fetch_rejects_sender_leaf_spoofing_with_mismatched_public_key`, `tests::barrier_resolve_revoked_occupancies_rejects_deployment_profile_manifest_mismatch_across_pages`, `tests::barrier_resolve_join_occupancies_since_rejects_global_history_attestation_mismatch_across_pages`, `tests::barrier_fetch_public_tree_rejects_deployment_profile_manifest_mismatch_across_pages` in [`../../crates/cityg-api-client/src/lib.rs`](../../crates/cityg-api-client/src/lib.rs) |
...
- Optional scale-up runners now exist for longer confidence-building campaigns:
...
- Useful future work is now optional scale-up on top of the named runners above:
  - extending the large-group chaos runner from `8-16` to `8-32` participants
    when host capacity is sufficient
  - broadening the current property-style mutation harness to more bundle types
    and larger mutation domains
  - longer soak windows on top of the combined restart traffic runner
