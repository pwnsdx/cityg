...
- `7.4` `Partial` — the base profile still lacks a mandatory non-omission proof, but `local-history-authority-v1` now defines and implements signed helper completeness attestations for A/B/C within one deployment-local scope, and the client positively verifies them when that extension is present.
  Proof: `docs/specs.md` (S3.3 shared authenticated-view rule and S3.3.E), `crates/cityg-server/src/lib.rs:2644-2698,3332-3421`, `crates/cityg-api/src/lib.rs:2607-2852`, `crates/cityg-api-client/src/lib.rs:2787-2894`, `crates/cityg-api-client/tests/history_authority_extensions.rs`.
