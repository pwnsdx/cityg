...
- `4.1` `Partial` — the base profile still relies mostly on honest-client enforcement, but the negotiated history-authority extensions now make updater eligibility server-checkable inside one deployment-local or deployment-global scope through mandatory `header[181]` + `header[182]`.
  Proof: `docs/specs.md` (S3.3.E-F, S11.11.4-S11.11.5), `crates/cityg-server/src/lib.rs:4644-4731,7025-7059,7192-7234`, `crates/cityg-api-client/tests/history_authority_extensions.rs`, `crates/cityg-gui/src/native/tests/mod.rs:1177-1254`.
