...
use cityg_core::kem::{KemSecret, validate_public_key};
use rand_core::CryptoRngCore;

use crate::crypto::{
    SEALED_SECRET_BYTES, Secret, Wrap, chain, fresh_secret, node_key, unwrap, wrap,
};
    ...
    for (planned, update) in plan.nodes.iter().zip(updates) {
        ...
        for target in &planned.wrap_to {
            ...
            if wrapped.node != planned.node
                ...
                || wrapped.kem_ciphertext.len() != cityg_core::kem::KEM_CIPHERTEXT_BYTES
            {
