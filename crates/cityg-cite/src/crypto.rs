//! Hashing, key derivation and wraps of profile v0.4-draft.
//!
//! The functions of profile v0.3 (docs/specs.md section 4) with v0.4 tags,
//! so that no hash, key or MAC of one profile can be taken for one of the
//! other:
//!
//! * `H_L(label, args) := BLAKE3(CBOR_det(["city-g/v0.4", label, args]))`;
...
//! * `Extract(salt, ikm) := BLAKE3-keyed(key = salt, ikm)`, as in v0.3.
//!
...
use cityg_core::hash::{Digest, extract, h};
use cityg_core::kem::{KEM_CIPHERTEXT_BYTES, KemSecret, encapsulate};
...
pub const PROFILE: &str = "city-g/v0.4-draft";
...
pub const KEM_WRAP_BYTES: usize = cityg_core::kem::KEM_CIPHERTEXT_BYTES + SEALED_SECRET_BYTES + 16;
...
mod tests {
    ...
    #[test]
    fn tags_differ_from_profile_v03() {
        let v04 = h_l("tree/leaf", vec![]).unwrap();
        let v03 = cityg_core::hash::h_l("tree/leaf", vec![]).unwrap();
        assert_ne!(v04, v03);
        let key = [7u8; 32];
        assert_ne!(
            *derive_secret(&key, "epoch").unwrap(),
            *cityg_core::hash::derive_secret(&key, "epoch").unwrap()
        );
    }
