...
fn a_joiner_applies_a_pending_removal_when_nobody_is_online() {
    ...
    assert!(
        task.changes
            .iter()
            .any(|c| c.leaf == target.leaf && c.kind == cityg_cite::objects::ChangeKind::Join)
    );
    assert!(!sim.members.contains_key(&target));
    assert!(!sim.ds.state().tree.is_member(target));
    sim.assert_agreement();
...
mod forged {
    use cityg_cite::commit::{Change, DistrictCommit, DistrictCommitContent, EntrantInit};
    use cityg_cite::crypto::kem_pk_hash;
    use cityg_cite::objects::{Admission, ChangeKind, JoinRequest, Request, device_id};
    use cityg_cite::packet::{EntrantEvidence, EntrantProof, Packet, RegistryUpdate};
    use cityg_cite::rekey::{KeySource, WindowIndex, generate, plan_district};
    use cityg_cite::roles::{SealDraft, build_city, finish_seal, with_city};
    use cityg_cite::schedule::external_init;
    use cityg_cite::tree::{Occupancy, Overlay, TreeDelta};
    ...
    pub struct Forged {
        ...
        pub seal: cityg_cite::commit::Seal,
    ...
    pub fn unchecked_commit(
        ...
        hedge: &[u8; 32],
        rng: &mut ChaCha20Rng,
    ) -> (DistrictCommit, cityg_cite::rekey::Rekeyed) {
...
fn run_window_keeping_secrets(
    sim: &mut Sim,
    committer: cityg_cite::tree::Occupancy,
) -> Vec<cityg_cite::rekey::Rekeyed> {
    use cityg_cite::roles::build_district;
    sim.now += 60_000;
...
fn removing_a_committer_rekeys_the_nodes_it_drew() {
    use cityg_cite::crypto::{node_key, unwrap};
    ...
    let n31 = cityg_cite::tree::NodeId { level: 3, index: 1 };
    let wrap31 = sim
        .ds
        .window(c_epoch)
        .unwrap()
    ...
    let mut known: Vec<cityg_cite::crypto::Secret> = vec![s31.clone()];
    for drawn in &kept {
        for level in 1..=shape.height {
            for index in 0..(1u32 << (shape.height - level)) {
                let node = cityg_cite::tree::NodeId { level, index };
    ...
    // secret C holds.
    let without_rule = cityg_cite::rekey::plan_city(
        &before.tree,
        shape,
...
fn the_delivery_service_evicts_only_under_an_admin_policy() {
    ...
    assert_eq!(
        task.changes
            .iter()
            .filter(|c| c.kind == cityg_cite::objects::ChangeKind::Eviction)
            .count(),
        3
    );
    for member in idle {
...
fn audits_expose_a_committer_that_placed_an_invalid_join() {
    use cityg_cite::audit::{self, FraudProof, Verdict};
    use cityg_cite::commit::Change;
    use cityg_cite::objects::{Admission, ChangeKind, JoinRequest, Request, device_id};
    use cityg_cite::roles::WindowTask;
    use cityg_cite::window::{Requests, WindowShape, check_window, needed_height};
    use cityg_core::identity::DeviceIdentity;
    use cityg_core::kem::KemSecret;
...
fn invites_admit_joiners_up_to_their_uses_and_expiry() {
    ...
    use cityg_cite::objects::Admission;
    use cityg_core::identity::DeviceIdentity;
...
fn joiners_check_the_chain_of_seals_from_their_checkpoint() {
    use cityg_cite::member::Joiner;
    use cityg_cite::packet::SealerEvidence;
