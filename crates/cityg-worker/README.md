...
- the remaining realtime gap is transport hardening rather than basic routing: the Worker websocket path now has attachment-backed session metadata and a lightweight heartbeat, but it still does not emulate the native lag accounting behavior yet
