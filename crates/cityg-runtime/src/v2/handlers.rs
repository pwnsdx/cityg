...
use cityg_server::v2::{LogBody, LogEntry, Room, RoomConfig, RoomError, RoomRecord, RoomStore};
use prost::Message;
...
pub enum NativeRoomStore {
    Memory(cityg_server::v2::MemoryRoomStore),
    File(cityg_server::v2::FileRoomStore),
}

#[cfg(not(target_arch = "wasm32"))]
impl NativeRoomStore {
    /// Store under `state_path`'s sibling directory `<state_path>.v2.d`, or in
    /// memory when there is no state path.
    pub fn for_state_path(state_path: Option<&std::path::Path>) -> std::io::Result<Self> {
        match state_path {
            None => Ok(Self::Memory(cityg_server::v2::MemoryRoomStore::new())),
            Some(path) => {
                let mut root = path.as_os_str().to_os_string();
                root.push(".v2.d");
                cityg_server::v2::FileRoomStore::new(std::path::PathBuf::from(root))
                    .map(Self::File)
...
impl RoomStore for NativeRoomStore {
    ...
    ) -> Result<Option<cityg_server::v2::StoredRoom>, Self::Error> {
        match self {
