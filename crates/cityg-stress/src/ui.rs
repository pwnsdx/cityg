...
fn draw_metrics(frame: &mut Frame<'_>, area: Rect, app: &AppState) {
    let rows = vec![
        Row::new(vec![
            ...
            Cell::from("Commits ok"),
            Cell::from(app.metrics.commits_ok.to_string()),
        ]),
        Row::new(vec![
            Cell::from("Commit p50"),
            Cell::from(metric_text(app.metrics.commit_p50_ms)),
            Cell::from("Commit p95"),
            Cell::from(metric_text(app.metrics.commit_p95_ms)),
            Cell::from("Commit p99"),
            Cell::from(metric_text(app.metrics.commit_p99_ms)),
        ]),
        Row::new(vec![
            Cell::from("Commit conflicts"),
            Cell::from(app.metrics.commit_conflicts.to_string()),
            Cell::from("Capacity"),
