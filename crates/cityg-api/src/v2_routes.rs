//! `/v2` routes: the City-G v0.2 delivery service over HTTP.
//!
...
async fn subscription(socket: WebSocket, state: V2State, gid: Digest, head: u64) {
        ...
        tokio::select! {
            notice = heads.recv() => match notice {
                ...
                Err(broadcast::error::RecvError::Lagged(skipped)) => {
                    ...
                    warn!("v2 subscription lagged by {skipped} notices");
...
/// The `/v2` router.
pub fn router(state: V2State) -> Router {
