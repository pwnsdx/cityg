...
impl Serdes for Poly32 {
    // NOTE: current packing is straightforward; more compact encodings can
    // be introduced once interoperability constraints are known.
    fn serialize<W: Write>(&self, writer: &mut W) -> Result<()> {
...
impl Serdes for Proof {
    fn serialize<W: Write>(&self, writer: &mut W) -> Result<()> {
        ...
        // NOTE: challenge packing reuses the trinary encoder for simplicity.
