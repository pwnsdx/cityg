...
impl ConstantTimeEq for Poly256 {
    fn ct_eq(&self, other: &Self) -> Choice {
        let mut acc: i64 = 0;
        for (a, b) in self.coeff.iter().zip(other.coeff.iter()) {
            acc |= a ^ b;
        }
...
impl PolyArith for Poly256 {
    ...
    fn mul(a: &Self, b: &Self) -> Self {
        // NTT support lives in `ntt.rs`; this module focuses on coefficient-form utilities.
        school_book(a, b)
    }
...
pub(crate) fn poly256_inner_product(a: &[Poly256], b: &[Poly256]) -> Poly256 {
    if a.len() != b.len() {
        debug_assert!(
            a.len() == b.len(),
            "poly256_inner_product length mismatch ({} vs {})",
            a.len(),
            b.len()
        );
    }
    ...
    for (lhs, rhs) in a.iter().zip(b.iter()) {
        res.add_assign(&Poly256::mul(lhs, rhs));
    }
    res.normalized();
    res
}
...
pub(crate) fn poly256_inner_product_trinary(a: &[Poly256], b: &[Poly256]) -> Poly256 {
    if a.len() != b.len() {
        debug_assert!(
            a.len() == b.len(),
            "poly256_inner_product_trinary length mismatch ({} vs {})",
            a.len(),
            b.len()
        );
    }
    let mut res = Poly256::zero();
    for (lhs, rhs) in a.iter().zip(b.iter()) {
        res.add_assign(&Poly256::mul_trinary(lhs, rhs));
    }
    res.normalized();
    res
}
