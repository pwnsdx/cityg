...
fn poly256_strategy() -> impl Strategy<Value = Poly256> {
    prop::collection::vec(0..Q, Poly256::DEGREE).prop_map(|coeffs| {
        let mut arr = [0i64; Poly256::DEGREE];
...
fn poly256_trinary_strategy() -> impl Strategy<Value = Poly256> {
    prop::collection::vec(-1i64..=1i64, Poly256::DEGREE).prop_map(|coeffs| {
        let mut arr = [0i64; Poly256::DEGREE];
        for (i, value) in coeffs.into_iter().enumerate() {
            arr[i] = value;
        }
...
proptest! {
    #[test]
    fn add_commutative(a in poly256_strategy(), b in poly256_strategy()) {
        let lhs = Poly256::add(&a, &b);
        let rhs = Poly256::add(&b, &a);
        prop_assert_eq!(lhs.coeff, rhs.coeff);
    }

    #[test]
    fn mul_trinary_matches_general(a in poly256_strategy(), b in poly256_trinary_strategy()) {
        let trinary = Poly256::mul_trinary(&a, &b);
        let general = Poly256::mul(&a, &b);
        for i in 0..Poly256::DEGREE {
            let diff = trinary.coeff[i] - general.coeff[i];
            prop_assert_eq!(diff.rem_euclid(Q), 0);
        }
    }
