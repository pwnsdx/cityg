...
fn test_serdes_param() {
    ...
    println!("{:02x?}", buf);
    let param2 = Param::deserialize(&mut buf[..].as_ref()).expect("deserialize param");
    assert_eq!(param, param2);
}

#[test]
fn paramgen_with_rng_matches_seed_api() {
    let seed = [0x42u8; 32];
    let via_seed = <LBVRF as VRF>::paramgen(seed).expect("paramgen");

    let mut rng = ChaCha20Rng::from_seed(seed);
    let via_rng = LBVRF::paramgen_with_rng(&mut rng).expect("paramgen_with_rng");

    assert_eq!(via_seed, via_rng);
}

#[test]
fn paramgen_with_rng_advances_rng_state() {
    let seed = [0xA5u8; 32];
    let mut rng = ChaCha20Rng::from_seed(seed);
    let _ = LBVRF::paramgen_with_rng(&mut rng).expect("paramgen_with_rng");
